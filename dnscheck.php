...
// Fungsi untuk mendapatkan negara berdasarkan IP menggunakan API
function get_country_by_ip($ip) {
    $url = "http://ipinfo.io/{$ip}/json"; // API ipinfo.io
...
echo "<tr><th>No</th><th>DNS Server IP</th><th>ISP Name</th><th>Country</th><th>Domain</th><th>Record Type</th><th>Propagated</th></tr>";

$no = 1;
foreach ($dns_servers as $server_ip => $isp_name) {
    $country = get_country_by_ip($server_ip);  // Menambahkan negara berdasarkan IP
    $result = check_dns_propagation($domain, $server_ip);
    $record_type = $result["record_type"];
    $propagated = $result["ips"];
    
    // Menampilkan hasil dalam tabel
    echo "<tr><td>$no</td><td>$server_ip</td><td>$isp_name</td><td>$country</td><td>$domain</td><td>$record_type</td><td>$propagated</td></tr>";
    $no++;
}
